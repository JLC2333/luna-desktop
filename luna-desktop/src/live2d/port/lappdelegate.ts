...
export class LAppDelegate {
  ...
  /** 获取第一个 Subdelegate（Luna 切换模型用） */
  public getSubdelegate(): LAppSubdelegate | null {
    return this._subdelegates?.[0] ?? null;
  }
  ...
  private initializeSubdelegates(): void {
    ...
    for (let i = 0; i < LAppDefine.CanvasNum; i++) {
      ...
      // 外部から canvas が指定されていれば (#live2d)、それを優先して使う
      if (i === 0 && LAppDefine.Canvas != null) {
        canvas = LAppDefine.Canvas;
      } else {
        canvas = document.createElement('canvas');
        canvas.style.width = `${100 / LAppDefine.CanvasNum}vw`;
