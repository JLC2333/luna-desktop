...
export class LAppSubdelegate {
  ...
  public onPointBegan(pageX: number, pageY: number): void {
    ...
    // position:fixed な canvas では offsetLeft/offsetTop が 0 になる
  ...
  public onPointMoved(pageX: number, pageY: number): void {
    // position:fixed な canvas では offsetLeft/offsetTop が 0 になるため
    // getBoundingClientRect() で正しい座標を計算する
    const rect = this._canvas.getBoundingClientRect();
    const localX: number = pageX - rect.left;
    const localY: number = pageY - rect.top;
